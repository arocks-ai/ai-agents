...
def normalize_skill_name(name: str) -> str:
    """Normalizes skill names for robust comparison (hyphens vs spaces)."""
...
async def find_best_skill(user_message: str, available_skills: list) -> dict | None:
    ...
    prompt = (
        ...
        "MANIFEST:\n"
        f"{skills_text}\n"
        f"VALID OPTIONS: {', '.join(valid_names)}, NONE\n\n"
        "ROUTING RULES:\n"
        "1. MATCH ONLY if the skill's description or name explicitly covers the core technical intent of the request.\n"
        "2. DISAMBIGUATION: If multiple skills overlap, pick the most specific one.\n"
        "3. LEARNING TRIGGER: If the request is about a completely new technical topic or library NOT in the manifest, output 'NONE'.\n"
        "4. OUTPUT FORMAT: Output ONLY the raw skill name string or 'NONE'. No quotes, no periods.\n\n"
    ...
    try:
        ...
        for s in available_skills:
            ...
            # if normalize_skill_name(skill_name) == normalize_skill_name(text):
            if normalize_skill_name(text) in normalize_skill_name(skill_name):
                return s            
